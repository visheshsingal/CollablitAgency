...
export default function AdminSidebar() {
  ...
  return <aside className="admin-sidebar"><div className="sidebar-brand"><BrandLogo compact /><strong>Operations</strong></div><nav>{items.map((item) => <Link href={item.href} className={router.pathname === item.href ? 'active' : ''} key={item.href}><i>{item.icon}</i>{item.label}</Link>)}</nav><style jsx>{`
    ...
    .sidebar-brand { display:flex; align-items:center; gap:9px; color:#fff; padding: 5px 9px 20px; border-bottom:1px solid rgba(255,255,255,.14); }.sidebar-brand :global(.brand-logo) { display:block; width:125px; height:34px; object-fit:contain; object-position:left center; filter:brightness(0) invert(1); }.sidebar-brand strong { font-size: .82rem; letter-spacing:.03em; }
